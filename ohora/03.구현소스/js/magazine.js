...
function loadFn() {
    ...
    goSlide = (seq) => {
        ...
        // [변경1]슬라이드 부모박스 width값 -> %이동값을 px변환 위해!
        ...
        if (seq) {
            ...
            // (1-2) 동시에 left값을 -200%으로 변경한다!
            // slide.style.left = "-200%";
            // [변경2]-> px변환+드래그값 보정!
            slide.style.left = -(sldParent*2-rx)+"px";
            // (1-3) 트랜지션 없애기!
            ...
            // 변경효과가 없음!!!
            setTimeout(() => {
                slide.style.left = -(sldParent*3)+"px";
        ...
        // 1-2. 왼쪽버튼 클릭시 //////////////
            ...
            // (2) 동시에 left값을 -400%로 변경한다.
            // slide.style.left = "-400%";
            // [변경3]-> px변환+드래그값 보정!
            slide.style.left = -(sldParent*4-rx)+"px";

            // 이때 트랜지션을 없앤다(한번실행후 부터 생기므로!)
            slide.style.transition = "none";

            // (3) 그 후 left값을 -300%으로 애니메이션하여
            // 슬라이드가 왼쪽에서 들어온다.
...
function goDrag(obj) {
    ...
    const dMove = () => {
        ...
        if (drag) {
            ...
            console.log(event.changedTouches,mvx);
...
function goWhere(obj) {
    ...
    // 2. 부모박스를 기준한 -300% left 위치값 구하기
    let tgPoint = obj.parentElement.clientWidth * 3;
